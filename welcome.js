...
// });
  ...
  // var str="patna";
        ...
        if(i.readyState==4 )
        {
          myObj = JSON.parse(this.responseText); 
               ...
               console.log(B);
                ...
                marker.bindPopup("My Location "+str).openPopup();
     ...
     // $("#form1").serialize(), 
     // http://localhost/deep-crop-yield-prediction/newpage.php
     // crop: $('#cropName').val(),
...
$.each(options, function(i, p) {
    $('#crop').append($('<option></option>').val(p).html(p));
});
...
// var options3=["Season", "1"];
var options3=["Season     ","Kharif     ", "Rabi       ", "Whole Year ", "Autumn     ", "Summer     ", "Winter     "];
$('#season').empty();
$.each(options3, function(i, p) {
    $('#season').append($('<option></option>').val(p).html(p));
});

var options4=["States","ARUNACHAL PRADESH", "BIHAR", "CHHATTISGARH", "HIMACHAL PRADESH", "JHARKHAND", "KERALA", "PUNJAB", "TAMIL NADU", "UTTARAKHAND"];
$('#state_name').empty();
$.each(options4, function(i, p) {
    $('#state_name').append($('<option></option>').val(p).html(p));
});